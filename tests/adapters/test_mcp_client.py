...
class TestSSEConnectDisconnect:
    ...
    @pytest.fixture()
    def _mock_sse_sdk(self) -> Any:
        """Patch the mcp SDK objects used by SSEMCPClient."""
        mock_session = AsyncMock()
        mock_session.initialize = AsyncMock()
        mock_session.__aenter__ = AsyncMock(return_value=mock_session)
        mock_session.__aexit__ = AsyncMock(return_value=False)
    ...
    async def test_sse_client_connect_disconnect(self, _mock_sse_sdk: Any) -> None:
        ...
        # Verify sse_client was called with the stored params
        _mock_sse_sdk["sse_client"].assert_called_once_with(
            client._url,
            headers=client._headers,
            timeout=client._timeout,
            sse_read_timeout=client._sse_read_timeout,
        )
        # Verify ClientSession was created with read/write streams
        _mock_sse_sdk["ClientSession"].assert_called_once_with(
            _mock_sse_sdk["read"], _mock_sse_sdk["write"]
        )
        # Verify session was initialized
        _mock_sse_sdk["session"].initialize.assert_awaited_once()

        await client.disconnect()

        # Verify cleanup
        _mock_sse_sdk["session"].__aexit__.assert_awaited_once()
        _mock_sse_sdk["cm"].__aexit__.assert_awaited_once()
...
class TestSSECallToolError:
    """Verify call_tool wraps exceptions in MCPError."""
...
class TestSSEConnectError:
    """Verify SSE connection failure wraps in MCPError."""

    @pytest.mark.asyncio()
    async def test_sse_connect_error(self) -> None:
        """Connection failure wraps in MCPError(MCP_CONNECTION_FAILED)."""
        mock_cm = AsyncMock()
        ...
        with patch(
            ...
            with pytest.raises(MCPError, match="Failed to connect") as exc_info:
                await client.connect("sse://bad-server")
            assert exc_info.value.code == MCP_CONNECTION_FAILED
...
class TestSSEProtocolConformance:
    """Verify SSEMCPClient satisfies IMCPClient protocol."""
    ...
    def test_satisfies_imcp_client_protocol(self) -> None:
        ...
        # Verify all required methods exist with correct names
        for method_name in ("connect", "list_tools", "call_tool", "disconnect"):
            assert hasattr(SSEMCPClient, method_name), f"SSEMCPClient missing {method_name}"
    ...
    def test_isinstance_check_with_runtime_checkable(self) -> None:
        ...
        for method_name in ("connect", "list_tools", "call_tool", "disconnect"):
            assert hasattr(SSEMCPClient, method_name)
            assert hasattr(IMCPClient, method_name)
